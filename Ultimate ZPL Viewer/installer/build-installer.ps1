# ============================================================================
#  build-installer.ps1 — Publie l'application et génère le Setup.exe (Inno Setup).
#
#  Fait tout en une commande :
#    1) dotnet publish (Release, self-contained, non packagé)
#    2) copie le fichier de ressources .pri de l'app dans le dossier publish
#       (WinUI le génère sous bin\x64\Release mais ne le copie pas dans publish —
#        sans lui, l'app plante au démarrage : « Cannot locate resource … MainWindow.xaml »)
#    3) retire tout dossier de données WebView2 qui traînerait dans publish
...
Write-Host '==> 3/4  Nettoyage d''un éventuel dossier WebView2…' -ForegroundColor Cyan
...
Write-Host ''
Write-Host "OK — Setup.exe généré dans : $(Join-Path $PSScriptRoot 'Output')" -ForegroundColor Green
